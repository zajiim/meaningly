...
FutureOr<List<MapEntry<String, String>>> offlineWords(Ref ref, String query) async {
  ...
  final result = await localDataSource.getOfflineWords(query: query);
  stopwatch.stop();
  debugPrint('Offline words fetch took ${stopwatch.elapsedMilliseconds}ms for query "$query" (Returned ${result.length} items)');
  return result;}